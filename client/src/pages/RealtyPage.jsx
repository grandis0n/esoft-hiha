...
const RealtyPage = () => {
    ...
    const [stringToCoordinates, setStringToCoordinates] = useState("");
    const [typeOfRealty, setTypeOfRealty] = useState(1);
    const [filterTypeOfRealty, setFilterTypeOfRealty] = useState(0);
    ...
    async function filterRealtys(){
        ...
        if (filterTypeOfRealty !== 0) {
            body.type_id = filterTypeOfRealty
        }
    ...
    async function filterRealtysByCoordinates(){
  
        let body = {}

        let a =  stringToCoordinates.split(", [").map(el=>el.replace('[', '').replace("]", ''))
        ...
        setRealtys(CoordinatesRealtys.data)
    };
            ...
            <div className="filter">
                <Box sx={{ minWidth: 250 }}>
                    <FormControl fullWidth>
                        ...
                        value={filterTypeOfRealty}
                        label="Age"
                        onChange={e=>{setFilterTypeOfRealty(e.target.value)}}
                        >
                        <MenuItem value={0}>Всё</MenuItem>
                ...
                <DeleteButton onClick={()=>handleOutOfSearch()}>Сбросить фильтр</DeleteButton>
            </div>
            <div className="folter__coordinates">
                <MyInput type="text" placeholder="Введите координаты" value={stringToCoordinates} onChange={e => setStringToCoordinates(e.target.value)}></MyInput>
                <MyButton onClick={()=>filterRealtysByCoordinates()}>Фильтровать</MyButton>
                <DeleteButton onClick={()=>handleOutOfSearch()}>Сбросить фильтр</DeleteButton>
            </div>
